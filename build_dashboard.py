...
def build_vlm_attr_table(vlm_per_attr: dict) -> str:
    ...
    for attr, d in vlm_per_attr.items():
        yes_c = d.get("yes_conf", d.get("score"))
        no_c  = d.get("no_conf")
        resp  = str(d.get("response", ""))[:80]
        rows += f"""<tr>
            <td>{h(attr)}</td>
            <td style="color:{score_color(yes_c)};font-weight:600">{fmt(yes_c)}</td>
            <td style="color:{score_color(no_c)}">{fmt(no_c)}</td>
...
def build_clip_attr_table(clip_per_attr: dict) -> str:
    ...
    for attr, d in clip_per_attr.items():
        gen_v = d.get("gen_score")
        ref_v = d.get("ref_score")
        delta = d.get("delta")
        if delta is None and isinstance(gen_v, (int, float)) and isinstance(ref_v, (int, float)):
            delta = gen_v - ref_v
        d_color = "#3fb950" if isinstance(delta, (int, float)) and delta >= -0.03 else "#f85149"
        sign = "+" if isinstance(delta, (int, float)) and delta >= 0 else ""
        rows += f"""<tr>
            <td>{h(attr)}</td>
            <td style="color:{score_color(gen_v)}">{fmt(gen_v)}</td>
            <td>{fmt(ref_v)}</td>
...
# ─────────────────────────────────────────────────────────────
# HTML: blocco verify completo (rejected_concepts.json)
# ─────────────────────────────────────────────────────────────
...
def build_attempt_details(attempt: dict) -> str:
    ...
    vlm_pa   = attempt.get("vlm_per_attribute", {}) or {}
    clip_pa  = attempt.get("clip_per_attribute", {}) or {}
...
def build_timeline(concept: str, safe_name: str, status: str,
...
def build_recovered_card(concept: str, safe_name: str,
    ...
    last_method     = attempts_log[-1].get("method", "?") if attempts_log else "?"

    fail_tags = "".join(f'<span class="attr-tag attr-fail">{h(a)}</span>' for a in original_fail)

    ref_cell  = img_cell("📷 Reference", ref_path, assets_abs, assets_rel, safe_name, "ref")
    timeline  = build_timeline(concept, safe_name, "recovered",
    ...
    for att in attempts_log:
        ...
        label   = f"Attempt {n} — {h(method)} — {'✅ SUCCESS' if success else '❌ fail'}"
