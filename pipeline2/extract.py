    ...
    prompt = """Analyze this retail product image and extract detailed visual attributes in JSON format:
{
  ...
  "distinctive_features": "unique visual elements"
    ...
    try:
        ...
        print(f"   ✅ Fingerprints estratte ({len(answer)} chars)")
        fingerprints_dict = parse_fingerprints(answer)
        print(f"   Preview: {answer[:200]}...")
        print_memory_stats("DOPO inferenza")
        
        return fingerprints_dict, model
...
def parse_fingerprints(raw_text):
    """
    Estrae il JSON dalla risposta del VLM (anche se ha testo extra).
    """
    try:
        ...
        if json_match:
            ...
            # Fallback: crea dizionario da testo grezzo
            print("   ⚠️ JSON non trovato, parsing manuale...")
            return {"description": raw_text.strip()}
            
    except json.JSONDecodeError:
        print("   ⚠️ JSON malformato, uso testo grezzo")
        return {"description": raw_text.strip()}
