...
# Usiamo il path assoluto di python del tuo ambiente Conda in modo coerente
...
OUTPUT_DIR=/leonardo_work/IscrC_MUSE/tballari/FM_Data/output/ablation_full_F_full_textdriven
DATABASE=database/database_textdriven.json
FLUX_PORT=8766
...
if [ "$REJECTED_COUNT" -eq 0 ]; then
    ...
    CUDA_VISIBLE_DEVICES=1 $CONDA_PYTHON flux_server.py --port $FLUX_PORT \
        > "logs/ablation_F/flux_server_${SLURM_JOB_ID}.log" 2>&1 &
    FLUX_PID=$!
