# test/anydoor_vanilla.py
"""
TEST 1: AnyDoor Vanilla (without fingerprints)

Generate images using AnyDoor with just the reference image,
without using extracted fingerprints or optimized prompts.

This tests AnyDoor's native object customization capability.

Usage:
    python test/anydoor_vanilla.py --category bag --num 5
    python test/anydoor_vanilla.py --category bottle --num 10
"""
...
from test.utils_test import (
    ...
    load_reference_image, save_experiment_metadata, ExperimentLogger
)
from test.anydoor_wrapper import AnyDoorWrapper


class AnyDoorVanillaGenerator:
    """
    AnyDoor generator without fingerprints.
    
    Uses only:
    - Reference image
    - Generic prompt based on category
    
    Does NOT use:
    - Extracted fingerprints
    - Optimized SDXL prompt
    - Layer-wise scaling (AnyDoor has its own mechanism)
    """
    ...
    def __init__(self, category: str = None, num_concepts: int = None):
        ...
        self.logger = ExperimentLogger(
            self.output_dir,
            f"AnyDoor Vanilla - {self.category}"
        )
    ...
    def _print_header(self):
        ...
        print(f"\n{'='*70}")
        print("🚪 TEST: AnyDoor Vanilla (No Fingerprints)")
        print(f"{'='*70}")
        print(f"   Category:     {self.category}")
        print(f"   Num Concepts: {self.num_concepts}")
        print(f"   Output:       {self.output_dir}")
        print(f"   Mode:         Reference image + generic prompt")
    ...
    def _generate_single(self, concept_id: str, content: dict) -> dict:
        """Generate a single image with AnyDoor."""
        result = {
        ...
        try:
            ...
            ref_img = load_reference_image(
                ref_image_path,
                size=512  # AnyDoor typically uses 512
            )
            
            # Generic prompt based on category
            category_name = content.get("category", "object")
            generic_prompt = f"a {category_name}, high quality, professional photo"
            ...
            output_path = os.path.join(self.output_dir, f"{safe_id}_anydoor.png")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test 1: AnyDoor Vanilla (No Fingerprints)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python test/anydoor_vanilla.py --category bag --num 5
    python test/anydoor_vanilla.py --category bottle --num 10
    python test/anydoor_vanilla.py --category cup
...
Note: 
    ...
    generator = AnyDoorVanillaGenerator(
