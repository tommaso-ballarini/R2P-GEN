...
Metrics used in DreamBench (Ruiz et al., 2023) for the paper table.

Calculates DINO-I, CLIP-I, CLIP-T with checkpoints aligned to the literature:
    - CLIP-I / CLIP-T : openai/clip-vit-base-patch32  (ViT-B/32)
    - DINO-I          : facebook/dino-vits16           (ViT-S/16, DINOv1)

Produce two output for table:
    - zero-shot : images generated by generate_dreambench.py, BEFORE
                  verify/refine (main run, comparable with DreamBooth and other zero-shot methods).
    - full      : final images after the recovery loop of refine_dreambench.py


Output:
    {output_dir}/metrics_dreambench.json   — results per image
    {output_dir}/metrics_summary.json      — averages per subject and global
    {output_dir}/metrics_table.txt         — table ready for the paper
"""
...
def _get_all_real_images(content: dict) -> list:
    """Returns ALL real images of the subject (for pairwise comparison).
    Uses content["image"] which the database populates already with the entire folder."""
    images = content.get("image", [])
    if isinstance(images, list) and images:
        return [p for p in images if os.path.exists(p)]
    # fallback to representative_image if for some reason "image" is empty
...
def evaluate(
    ...
    """Evaluates all generated images and returns the results dictionary."""

    print(f"\n{'='*70}")
    print("📊 Phase 4 — DREAMBENCH EVALUATION (DINO-I / CLIP-I / CLIP-T)")
    print(f"{'='*70}")
    ...
    print("   Loading metric models (official DreamBench checkpoints)...")
    ...
    for concept_id, content in tqdm(concept_dict.items(), desc="Concepts"):
        ...
        concept_name = _sanitize_folder_name(concept_id)  

        if not real_images:
            print(f"   ⚠️  {concept_id}: no real images found → skip.")
            continue
...
# ---------------------------------------------------------------------------
# Aggregation: mean per subject and global
# ---------------------------------------------------------------------------

def aggregate(per_image_results: dict) -> dict:
    """Calculate means per subject and global means.
    ...
    # Global means (mean of means per subject — DreamBooth protocol)
...
def run_evaluation(
    ...
    print(f"\n📄 Results per image → {per_image_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DreamBench Phase 4 — Evaluation")
    parser.add_argument("--database",     required=True,  help="Path to database_db.json")
    parser.add_argument("--output",       required=True,  help="Folder with generated images (output of generate_dreambench.py)")
    parser.add_argument("--results-dir",  required=True,  help="Folder where to save the metric JSONs")
    parser.add_argument("--label",        default="R2P-GEN", help="Label for the table (e.g., 'R2P-GEN (zero-shot)' or 'R2P-GEN (full)')")
