...
"""
BASELINE 3: Solo Descrizione Generale (campo "general")

Generazione usando la descrizione generale dal JSON (campo "general"),
combinata con IP-Adapter.

Confronta il prompt SDXL ottimizzato vs la descrizione grezza.

Usage:
    python baseline/general_desc_only.py --category bag --num 5
    python baseline/general_desc_only.py --category bottle --num 10
"""
...
class GeneralDescOnlyGenerator:
    """
    Generatore baseline: descrizione generale + IP-Adapter.
    
    Non usa:
    - Prompt SDXL ottimizzato
    - Layer-wise scaling
    - Fingerprints dettagliati
    
    Usa:
    - IP-Adapter con scale globale
    - Campo "general" del JSON come prompt
    """
    ...
    def _print_header(self):
        ...
        print(f"\n{'='*70}")
        print("💬 BASELINE 3: General Description + IP-Adapter")
        print(f"{'='*70}")
        print(f"   Category:    {self.category}")
        print(f"   Num Concepts: {self.num_concepts}")
        print(f"   Output:      {self.output_dir}")
        print(f"   Prompt:      'general' field from JSON")
    ...
    def _generate_single(self, concept_id: str, content: dict) -> dict:
        ...
        try:
            ...
            # Get GENERAL description (NOT optimized SDXL prompt!)
            info = content.get("info", {})
            general_desc = info.get("general", "")
            
            if not general_desc:
                result["error"] = "No general description in database entry"
                return result
            
            result["general_desc"] = general_desc[:100] + "..."  # Truncate for logging
            
            # Use general description as prompt (not optimized!)
            generated = self.pipe(
                prompt=general_desc,
            ...
            output_path = os.path.join(self.output_dir, f"{safe_id}_generaldesc.png")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Baseline 3: General Description + IP-Adapter",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python baseline/general_desc_only.py --category bag --num 5
    python baseline/general_desc_only.py --category bottle --num 10
    python baseline/general_desc_only.py --category cup
        """
    )
    parser.add_argument(
        "--category", "-c",
    ...
    generator = GeneralDescOnlyGenerator(
